...
ghcr.io/<owner>/<repository>:1.0.0         # v1.0.0 tag
...
```bash
SHAM_IMAGE=ghcr.io/<owner>/<repository>:1.0.0 docker compose pull
SHAM_IMAGE=ghcr.io/<owner>/<repository>:1.0.0 docker compose up -d
```

## 4. Create version 1.0.0

Confirm `package.json` contains `"version": "1.0.0"`, the lockfile is committed, and CI is green. Then create and push the tag:
...
- Pull `ghcr.io/<owner>/<repository>:1.0.0` on a clean host.
